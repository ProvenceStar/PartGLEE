...
if __name__ == "__main__":
    ...
    new_data = {"images": [], "annotations": [], "categories": [{"supercategory": "OBJECT", "id": 1, "name": "object"}, {"supercategory": "PART", "id": 2,"name": "part"}]}
    for file_name in tqdm(ann_file_names[:]):
        ...
        # shutil.copy( '{}/images/{}'.format(args.src, file_name.replace('json','jpg')),'sa1b_mini_val/{}'.format(file_name.replace('json','jpg')))
        ...
        part_valids = list(map(lambda x: not x, valids))
        part_idx = sort_idx[part_valids].tolist()
        per_image_new_ann = []
        # for idx in valid_idx: # range(len(ann['annotations'])):#valid_idx:  #只取没有被NMS掉的mask
        for idx in range(len(ann['annotations'])):
            ...
            elif idx in part_idx:
                anno = {"bbox":cur_data["bbox"], "segmentation":cur_data["segmentation"], "image_id":ann['image']['image_id'], \
                    "iscrowd":0, "category_id": 2, "id":cur_data['id'], "area": cur_data['area']}
        ...
        # json_name = '{}/{}'.format(args.src, file_name.replace('.', '_modified.'))
        os.remove(json_name)
