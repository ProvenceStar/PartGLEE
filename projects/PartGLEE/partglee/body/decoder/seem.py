...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.is_valid_object = MLP(hidden_dim, hidden_dim, 1, 3) # to distinguish if a query matched a valid object (for refcoco and sa1b and other calss agnostic task)

        self.class_embed = nn.Parameter(torch.rand(hidden_dim, dim_proj))
    ...
    def from_config(cls, cfg, in_channels, lang_encoder, mask_classification, extra):
        ...
        ret["task_switch"] = {'bbox': False, 'mask': {'ENABLED': True}, 'spatial': True, 'grounding': True, 'openimage': {'grounding': False, 'mask': False}, 'visual': False, 'audio': False}

        # attn data struct
        ret["attn_arch"] = {'VARIABLE': {'queries': ['object'], 
        'tokens': ['grounding', 'spatial', 'visual', 'audio']}, 
        'SELF_ATTENTION': {'queries': {'object': ['queries_object', 'tokens_grounding', 'tokens_spatial', 'tokens_visual', 'tokens_audio']}, 
                            'tokens': {'grounding': ['queries_object', 'tokens_grounding'], 'spatial': ['tokens_spatial'], 'visual': ['tokens_visual'], 'audio': ['queries_object', 'tokens_audio']}}, 
        'CROSS_ATTENTION': {'queries': {'object': True}, 'tokens': {'grounding': False, 'spatial': False, 'visual': False, 'audio': False}}, 'MASKING': ['tokens_spatial', 'tokens_grounding', 'tokens_visual', 'tokens_audio'], 'DUPLICATION': {'queries': {'grounding': 'queries_object', 'spatial': 'queries_object'}}, 
        'SPATIAL_MEMORIES': 32}
    ...
    def forward(self, x, mask_features, mask=None, target_queries=None, target_vlp=None, task='seg', extra={}):
        ...
        self.task_switch['spatial'] = 'spatial_query_pos_mask' in extra.keys()
        self.task_switch['grounding'] = 'grounding_tokens' in extra.keys()
        ...
        self.attention_data.set_results(results)  # 比如第一步初始化的attention mask，更新进attention里
        # import pdb;pdb.set_trace()
        
        ### 接下来是真正的decoder，包括 output和 src之间的cross attention，以及output自己或者与text/visual prompt 之间的selfattention（SEEM Fig4 中的过程）
        for i in range(self.num_layers):
            ...
            # import pdb;pdb.set_trace()  # output:[512+101, bz, 512]  query_embed:[512+101, bz, 512]   
            #这里将spatial_tokens 与本身的query_embed concat，一起做selfattention，对应文章中的通过self attention将prompt与 learnable query 交互
            ...
            output, query_embed = self.attention_data.cross_attn_variables()
            # import pdb;pdb.set_trace()   #  output, query_embed [101, bz, 512] 
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, layer_id=-1):
        ...
        decoder_output = decoder_output.transpose(0, 1)  #[bz,num_q,512]
        class_embed = decoder_output @ self.class_embed  # [bz,101,512]
        outputs_class = self.lang_encoder.compute_similarity(class_embed, name='lvis_categories')  #[bz,101,80] 这里算的是class embedding和 category embedding的相似性
        # import pdb;pdb.set_trace()
        mask_embed = self.mask_embed(decoder_output)
        object_score = self.is_valid_object(decoder_output) # [bz,num_q,1]
        ...
        results = {
            ...
            "predictions_object_score": object_score,
        }
        return results
