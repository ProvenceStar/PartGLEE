...
class YTVISEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        """
        if self.dataset_name.startswith("refytb-val") or self.dataset_name.startswith("rvos-refytb-val")\
...
def instances_to_coco_json_video(inputs, outputs):
    ...
    if isinstance(outputs,list): #tao
        ...
        ytvis_results = []
        for instance_id, (s, l, m, b) in enumerate(zip(scores, labels, masks, boxes)):
            # segms = [
            #     mask_util.encode(np.array(_mask[:, :, None], order="F", dtype="uint8"))[0]
            #     for _mask in m
            # ]
            # _boxes = [_b.tolist() for _b in b]

            segms = []
            _boxes = []
            for _mask,_box in zip(m,b):
                if _mask is None:
