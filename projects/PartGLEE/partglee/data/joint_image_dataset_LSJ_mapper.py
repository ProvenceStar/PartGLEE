...
class Joint_Image_LSJDatasetMapper:
    ...
    def __init__(self, cfg, is_train=True):
        ...
        if is_train:
            ...
            logging.getLogger(__name__).info(
                "[JointInstanceNewBaselineDatasetMapper] Full TransformGens used in training: {}".format(str(self.tfm_gens))
            )
    ...
    def __call__(self, dataset_dict):
        ...
        if "pan_seg_file_name" in dataset_dict:     # panoptic data
            image, transforms = T.apply_transform_gens(self.tfm_gens, image)
            image_shape = image.shape[:2]  # h, w
        ...
        else:  # detection dataset [coco obj365 UVO etc]
            # import pdb;pdb.set_trace()
            padding_mask = np.ones(image.shape[:2])
            image, transforms = T.apply_transform_gens(self.tfm_gens, image)
            # the crop transformation has default padding value 0 for segmentation
            ...
            if "annotations" in dataset_dict:
                ...
                for anno in dataset_dict["annotations"]:
                    ...
                    anno.pop("keypoints", None)
                if dataset_dict.get('task') == 'vg' or dataset_dict.get('task') =='vg_joint' or dataset_dict.get('task') =='grit':
                ...
                Max_inst = 500
                if len(annos) > Max_inst:
                    print(len(annos))
                    annos = annos[:Max_inst]
                    if dataset_dict.get('task') == 'vg' or dataset_dict.get('task') =='vg_joint' or dataset_dict.get('task') =='grit':
                        object_description_list = object_description_list[:Max_inst]
                ...
                if 'gt_masks' in instances._fields.keys() and dataset_dict.get('dataset_name') not in ['voc', 'partimagenet_joint', 'pascal_joint']:
                    instances.gt_boxes = instances.gt_masks.get_bounding_boxes()
                ...
                if dataset_dict.get('task') == 'vg' or dataset_dict.get('task') == 'vg_joint' or dataset_dict.get('task') =='grit': # filter empty description
                    ...
                    assert len(_mask) == len(object_description_list), print(len(_mask), print(len(object_description_list)))
                    for description, _m in zip(object_description_list,_mask):
                        if _m:
                            dataset_dict["object_descriptions"].append(description)
