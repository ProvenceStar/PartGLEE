...
ADE20K_OBJECT_CLASS_NAMES = ['airplane', 'armchair', 'bed', 'bench', 'bookcase', 'bus', 'cabinet', 'car', 'chair', 'chandelier', 'chest of drawers', 'clock', 'coffee table', 'computer', 'cooking stove', 'desk', 'dishwasher', 'door', 
...
ADE20K_PART_CLASS_NAMES = ["person's arm", "person's back", "person's foot", "person's gaze", "person's hand", "person's head", "person's leg", "person's neck", "person's torso", "door's door frame", "door's handle", "door's knob", 
...
def _get_partimagenet_metadata():
    ADE20K_JOINT_CLASS_NAMES = ADE20K_OBJECT_CLASS_NAMES + ADE20K_PART_CLASS_NAMES
    ADE20K_JOINT_CATEGORIES = [{"id": idx, "name": ADE20K_JOINT_CLASS_NAMES[idx]} for idx in range(len(ADE20K_JOINT_CLASS_NAMES))]
    id_to_name = {x['id']: x['name'] for x in ADE20K_JOINT_CATEGORIES}
...
_ADE20K_JOINT = {
    "ade20k_joint_train": ("ADE20KPart234/images/training", "ADE20KPart234/ade20k_joint_train.json"),
    "ade20k_joint_val": ("ADE20KPart234/images/validation", "ADE20KPart234/ade20k_joint_val.json"),
}

def register_all_ade20k_joint(root):
    for key, (image_root, json_file) in _ADE20K_JOINT.items():
        register_coco_instances(
            key,
            _get_partimagenet_metadata(),
            os.path.join(root, json_file) if "://" not in json_file else json_file,
            os.path.join(root, image_root),
            dataset_name_in_dict="ade20k",
            evaluator_type='coco'
        )
