...
joint_base_part_categories = []
for i in range(len(PASCAL_PART_CATEGORIES)):
    joint_base_part_categories.append({"id": PASCAL_PART_CATEGORIES[i]['id'] + len(PASCAL_OBJECT_CATEGORIES), "name": PASCAL_PART_CATEGORIES[i]['name']})
JOINT_VAL_CATGORIES = PASCAL_OBJECT_CATEGORIES + joint_base_part_categories

PASCAL_BASE_PART_CATEGORIES = [
    {"id": 1, "name": "aeroplane:body"},
    {"id": 2, "name": "aeroplane:wing"},
    {"id": 3, "name": "aeroplane:tail"},
    {"id": 4, "name": "aeroplane:wheel"},
...
PASCAL_BASE_OBJECT_CATEGORIES = [
    ...
    {"id": 9, "name": "motorbike"},
    {"id": 10, "name": "person"},
    {"id": 11, "name": "potted plant"},
    {"id": 12, "name": "sheep"},
]

joint_base_part_categories = []
for i in range(len(PASCAL_BASE_PART_CATEGORIES)):
    joint_base_part_categories.append({"id": PASCAL_BASE_PART_CATEGORIES[i]['id'] + len(PASCAL_BASE_OBJECT_CATEGORIES), "name": PASCAL_BASE_PART_CATEGORIES[i]['name']})
JOINT_BASE_CATGORIES = PASCAL_BASE_OBJECT_CATEGORIES + joint_base_part_categories
...
def _get_partimagenet_metadata(key):
    if 'open_vocabulary_val' in key:
        id_to_name = {x['id']: x['name'] for x in PASCAL_PART_CATEGORIES}
    else:
        id_to_name = {x['id']: x['name'] for x in JOINT_BASE_CATGORIES}
...
_PASCAL_PART = {
    "pascal_part_base_train": ("pascal_part/VOCdevkit/VOC2010/JPEGImages", "pascal_part/train_base_joint.json"),
    "pascal_part_open_vocabulary_val": ("pascal_part/VOCdevkit/VOC2010/JPEGImages", "pascal_part/val.json"),
...
def register_all_pascal_part_open_vocabulary(root):
    for key, (image_root, json_file) in _PASCAL_PART.items():
        if "parsed" in key:
            register_coco_instances(
                ...
                dataset_name_in_dict='pascal_part_parsed',
            )
        else:
            register_coco_instances(
                key,
                ...
                os.path.join(root, json_file) if "://" not in json_file else json_file,
                os.path.join(root, image_root),
                dataset_name_in_dict='pascal_open_vocabulary',
                evaluator_type='pascal_part',
            )