...
def _evaluate_predictions_on_lvis(
    ...
    logger = logging.getLogger('detectron2.stanything.evaluation.paco_evaluation')
    ...
    if eval_attributes:
        ...
        from .lvis_eval import LVISEval
    ...
    """
        Modified to report category-specific AP
    """
    test_obj = ['bicycle', 'dog']
    test_part_categories = []
    for x in lvis_eval.lvis_gt.dataset["categories"]:
        if ":" in x["name"] and x["name"].split(":")[0] in test_obj:
            test_part_categories.append(x["name"])
    
    test_part_categories = sorted(test_part_categories)
    all_part_id_to_obj_part_ids = {}
    for x in lvis_eval.lvis_gt.dataset["categories"]:
        if ":" in x["name"] and x["name"] in test_part_categories:
            all_part_id_to_obj_part_ids[x["name"]] = (x["id"])

    category_specific_results = []
    category_specific_results_ap50 = []
    for test_part in test_part_categories:
        ori_id = all_part_id_to_obj_part_ids[test_part]
        mapped_id = obj_cats_to_cont_id_eval[ori_id]
        ap = get_AP_from_precisions(precisions, mapped_id)
        category_specific_results.append(("{}".format(test_part), float(ap * 100)))
        ap50 = get_AP50_from_precisions(precisions, mapped_id)
        category_specific_results_ap50.append(("{}".format(test_part), float(ap50 * 100)))

    # tabulate it
    N_COLS = min(6, len(category_specific_results) * 2)
    results_flatten = list(itertools.chain(*category_specific_results))
    ...
    results_flatten_ap50 = list(itertools.chain(*category_specific_results_ap50))
