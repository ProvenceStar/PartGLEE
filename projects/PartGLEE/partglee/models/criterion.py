...
from ._INFERENCE_CONSTANTS import CRITERION_NUM_CLASSES
from ._CONSTANTS import OBJECT_LEVEL_DATASETS, NO_MASK_TASKS
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses,
                 ...
                 topk_object_queries=50, num_part_queries=10, box_restrictions=True):
        ...
        self.no_mask_tasks = NO_MASK_TASKS
        
        for object_task in self.object_level_datasets:
            fake_task = object_task + '_fake'
            if fake_task not in self.no_mask_tasks:
                self.no_mask_tasks.append(fake_task)
        # prepare for box restrictions
        self.topk_object_queries = topk_object_queries
        self.num_part_queries = num_part_queries
        self.box_restrictions = box_restrictions
        ...
        self.num_classes = CRITERION_NUM_CLASSES
    ...
    def federated_loss(self,outputs, targets, indices):
        ...
        for neg_score, ex_score, noex_score, target, indice in zip(pred_federate['neg'],pred_federate['ex'],pred_federate['noex'],targets,indices): 
            ...
            # target 'gt_ex_label', 'gt_noex_label',  
            # exhaustive
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, task, object_outputs, topk_object_queries_indices, use_dn, log=True):
        ...
        if len(idx[0]) == 0:
            losses = {
                'loss_ce': (src_logits*0).sum() + (outputs['pred_scores']*0).sum(), 
            }
            return losses
    ...
    def loss_conf(self, outputs, targets, indices, num_boxes, task, object_outputs, topk_object_queries_indices, use_dn, log=True):
        """Classification loss (Binary focal loss)
        targets dicts must contain the key "labels" containing a tensor of dim [nb_target_boxes]
        """
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, task, object_outputs, topk_object_queries_indices, use_dn):
        ...
        if self.box_restrictions:
            if '_part' in task and '_object' not in task and '_fake' not in task and use_dn == False:
                object_output_boxes = torch.gather(object_outputs['pred_boxes'], 1, topk_object_queries_indices.unsqueeze(-1).repeat(1, 1, 4))
                bs = object_output_boxes.shape[0]
                topk = self.topk_object_queries
                # Get the corresponding obejct pred boxes of matched part prediction boxes
                repeat_object_output_boxes = object_output_boxes.unsqueeze(2).repeat(1, 1, self.num_part_queries, 1).flatten(1, 2)
                object_pred_boxes = torch.clamp(repeat_object_output_boxes[idx], 0, 1)
                # Get matched part prediction boxes
                part_pred_boxes = torch.clamp(outputs['pred_boxes'][idx], 0, 1)
                object_part_box_restriction_loss = box_ops.object_part_box_iou(box_ops.box_cxcywh_to_xyxy(object_pred_boxes), box_ops.box_cxcywh_to_xyxy(part_pred_boxes))
                if torch.isnan(object_part_box_restriction_loss / (bs * idx[0].shape[0])):
                    losses['loss_box_restriction'] = (outputs['pred_boxes']*0).sum()
                else:
                    losses['loss_box_restriction'] = object_part_box_restriction_loss / (bs * idx[0].shape[0])
            else:
                losses['loss_box_restriction'] = (outputs['pred_boxes']*0).sum()
            
        return losses

    def loss_boxes_panoptic(self, outputs, targets, indices, num_boxes, task, object_outputs, topk_object_queries_indices, use_dn):
        """Compute the losses related to the bounding boxes, the L1 regression loss and the GIoU loss
           targets dicts must contain the key "boxes" containing a tensor of dim [nb_target_boxes, 4]
           The target boxes are expected in format (center_x, center_y, w, h), normalized by the image size.
        """
    ...
    def loss_masks(self, outputs, targets, indices, num_masks, task, object_outputs, topk_object_queries_indices, use_dn):
    ...
    def get_loss(self, loss, outputs, targets, indices, num_masks, task, object_outputs=None, topk_object_queries_indices=None, use_dn=False):
        loss_map = {
        ...
        return loss_map[loss](outputs, targets, indices, num_masks, task, object_outputs, topk_object_queries_indices, use_dn)

    def forward(self, outputs, targets, mask_dict=None, task=None, object_outputs=None, topk_object_queries_indices=None):
        ...
        # Preserve matched object indices
        ...
        for loss in self.losses:
            losses.update(self.get_loss(loss, outputs, targets, indices, num_masks, task, object_outputs, topk_object_queries_indices, use_dn=False))

        if self.dn != "no" and mask_dict is not None:
            l_dict={}
            for loss in self.dn_losses:
                # import pdb;pdb.set_trace()
                l_dict.update(self.get_loss(loss, output_known_lbs_bboxes, targets, exc_idx, num_masks*scalar, task, object_outputs, topk_object_queries_indices, use_dn=True))
        ...
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                for loss in self.losses:
                    l_dict = self.get_loss(loss, aux_outputs, targets, indices, num_masks, task, object_outputs, topk_object_queries_indices, use_dn=False)
                ...
                if i>=start:
                    if self.dn != "no" and mask_dict is not None:
                        ...
                        for loss in self.dn_losses:
                            l_dict.update(
                                self.get_loss(loss, out_, targets, exc_idx, num_masks * scalar, task, object_outputs, topk_object_queries_indices, use_dn=True))
        ...
        if 'interm_outputs' in outputs:
            ...
            for loss in self.losses:
                l_dict = self.get_loss(loss, interm_outputs, targets, indices, num_masks, task, object_outputs, topk_object_queries_indices, use_dn=False)
                l_dict = {k + f'_interm': v for k, v in l_dict.items()}
                losses.update(l_dict)

        return losses, matched_indices
