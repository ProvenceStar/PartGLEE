    ...
    def forward(self,
        ...
        if task in ['grounding', 'rvos'] or  'visual_prompt_tokens' in extra:
            if self_attn_mask is not None: # training with denoising query 
                ...
                # new_self_attn_mask[:,ori_size-300:,ori_size-300:] = False #denoising之外的设Flase
                # import pdb;pdb.set_trace()
                new_self_attn_mask[:,:ori_size,:ori_size] =  self_attn_mask.unsqueeze(0).repeat(tgt.shape[1],1,1) #denoising和matching 保持原mask
