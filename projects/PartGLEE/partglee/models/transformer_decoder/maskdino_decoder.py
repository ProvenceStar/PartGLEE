...
from .._CONSTANTS import PACO_CATEGORIES, OPENIMAGES_OBJECT_CATEGORIES
...
def build_transformer_decoder(cfg, in_channels, lang_encoder, mask_classification=True, is_part_decoder=False):
    """
...
class MaskDINODecoder(nn.Module):
    ...
    def __init__(
        ...
        self.num_classes = {
            ...
            'paco':531,
            'paco_object':75,
            'openimage_joint_object': 588,
            'ade20k': 278,
            'ade20k_object': 44,
            'ade20k_base_object': 33,
            'pascalvoc': 136,
            'pascalvoc_object': 20,
            'pascalvoc_base_object': 15,
            'partimagenet_joint': 51,
            'partimagenet_joint_object': 11,
            'partimagenet_parsed_object': 11,
            'pascal_open_vocabulary_object': 12,
            'pascal_part_parsed_object': 20,
            'sa1b_joint': 2,
            'sa1b_joint_object': 1,
            'pascal_joint':113,
            'pascal_joint_object': 20,
            # 'pascal_part': 93,
            'coco_panoptic': 133,
            'openimage':100,
            'lvis_clip':100,
        ...
        self.part_datasets_object_category_idx = {
            ...
            'ade20k_object': [idx for idx in range(44)],
            'ade20k_base_object': [idx for idx in range(33)],
            'pascalvoc_object': [idx for idx in range(20)],
            ...
            'sa1b_joint_object': [0],
            'pascal_joint_object': [idx for idx in range(20)],
            'pascal_open_vocabulary_object': [idx for idx in range(12)],
            'pascal_part_parsed_object': [idx for idx in range(11)],
            'pascal_open_vocabulary_val_object': [idx for idx in range(14)],
            'seginw_House-Parts_object': [0],
            'seginw_Airplane-Parts_object': [0],
            'seginw_Bottles_object': [0, 1],
            'vg_joint_object': [idx for idx in range(200)],
        }
        # output FFNs
        ...
        self.openimage_obejct_label_enc = nn.Embedding(588, hidden_dim)
        self.obj365_label_enc = nn.Embedding(100, hidden_dim)
        self.paco_label_enc = nn.Embedding(531, hidden_dim)
        self.paco_object_label_enc = nn.Embedding(75, hidden_dim)
        self.ade20k_label_enc = nn.Embedding(278, hidden_dim)
        self.ade20k_object_label_enc = nn.Embedding(44, hidden_dim)
        self.ade20k_base_object_label_enc = nn.Embedding(33, hidden_dim)
        self.pascalvoc_label_enc = nn.Embedding(136, hidden_dim)
        self.pascalvoc_object_label_enc = nn.Embedding(20, hidden_dim)
        ...
        self.sa1b_joint_object_label_enc = nn.Embedding(1, hidden_dim)
        self.pascal_joint_label_enc = nn.Embedding(113, hidden_dim)
        self.pascal_joint_object_label_enc = nn.Embedding(20, hidden_dim)
        self.pascal_open_vocabulary_object_label_enc = nn.Embedding(12, hidden_dim)
        ...
        self.label_enc = {
            ...
            'paco_object': self.paco_object_label_enc,
            'ade20k': self.ade20k_label_enc,
            'ade20k_object': self.ade20k_object_label_enc,
            'ade20k_base_object': self.ade20k_base_object_label_enc,
            'pascalvoc': self.pascalvoc_label_enc,
            'pascalvoc_object': self.pascalvoc_object_label_enc,
            'pascalvoc_base_object': self.pascalvoc_base_object_label_enc,
            ...
            'sa1b_joint_object': self.sa1b_joint_object_label_enc,
            'pascal_joint': self.pascal_joint_label_enc,
            'pascal_joint_object': self.pascal_joint_object_label_enc,
            'pascal_open_vocabulary_object': self.pascal_open_vocabulary_object_label_enc,
            'pascal_part_parsed_object': self.pascal_joint_object_label_enc,
            'coco_panoptic': self.coco_panoptic_label_enc,
            'openimage': self.obj365_label_enc,
            'openimage_joint_object': self.openimage_obejct_label_enc,
            'grit': self.obj365_label_enc,
            'vg': self.vg_label_enc,
            'vg_joint_object': self.vg_label_enc,
    ...
    def forward(self, x, mask_features, extra, task, masks, targets=None, custom_object_categories_idx=None):
        """
        ...
        predictions_mask = []
        if self.two_stage:
            # query selection[only for object queries]
            # output_memory:[bs,h*w,c=256], output_proposals:[bs,h*w,4]
            ...
            if task in ['grounding','rvos']:
                ...
                if self.unify_object_part and task not in self.object_level_datasets:
                    if 'custom' in task:
                        ...
                        object_category_idx = self.part_datasets_object_category_idx[task]
                    object_class_embed = extra['class_embeddings'][object_category_idx, :]
                    enc_outputs_class_unselected = torch.einsum("bqc,nc->bqn", class_embed, object_class_embed)  #[bz,n,80]
                else:
            ...
            conf_score, outputs_class, outputs_mask,_ = self.forward_prediction_heads(tgt_undetach.transpose(0, 1), mask_features, task, extra, mask_dict = None, custom_object_categories_idx=custom_object_categories_idx)
            tgt = tgt_undetach.detach()
            # Reserve tgt as object queries here for parsing it into part queries before denoising
            # tgt_undetach:[bs,nq,c=256]
            
            # object_queries = tgt_undetach
        ...
        if self.initial_pred:
            conf_score, outputs_class, outputs_mask, pred_federat = self.forward_prediction_heads(tgt.transpose(0, 1), mask_features, task, extra, mask_dict, self.training, custom_object_categories_idx=custom_object_categories_idx)
        ...
        for i, output in enumerate(hs):
            conf_score, outputs_class, outputs_mask, pred_federat = self.forward_prediction_heads(output.transpose(0, 1), mask_features, task, extra, mask_dict, self.training or (i == len(hs)-1), custom_object_categories_idx=custom_object_categories_idx)
            predictions_score.append(conf_score)
            predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask)
            predictions_federate.append(pred_federat)
        ...
        if mask_dict is not None:
            ...
            object_queries = hs[-1][:, mask_dict['pad_size']:, :]
        else:
        ...
        if self.unify_object_part:
            return out, mask_dict, object_queries, src_flatten
        else:
            return out, mask_dict

    def forward_prediction_heads(self, output, mask_features, task, extra, mask_dict, pred_mask=True, visual_P=False, custom_object_categories_idx=None):
        ...
        if task in ['grounding', 'rvos']:
            ...
            if self.unify_object_part and task not in self.object_level_datasets:
                if 'custom' in task:
                    ...
                    object_category_idx = self.part_datasets_object_category_idx[task]
                object_class_embed = extra['class_embeddings'][object_category_idx, :]
                outputs_class = torch.einsum("bqc,nc->bqn", class_embed, object_class_embed)  #[bz,n,80]
            else:
