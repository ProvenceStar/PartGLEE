...
class HungarianMatcher(nn.Module):
    ...
    @torch.no_grad()
    def memory_efficient_forward(self, outputs, targets, task, cost=["cls", "box", "mask"]):
        ...
        for b in range(bs):
            ...
            # Final cost matrix
            if self.panoptic_on or 'panoptic' in task:
                isthing = tgt_ids<80
    ...
    def linear_sum_assignment_with_inf(self, cost_matrix):
        ...
        if min_inf or max_inf:
            ...
            cost_matrix[np.isinf(cost_matrix)] = max_values
        return linear_sum_assignment(cost_matrix)
    
    @torch.no_grad()
    def forward(self, outputs, targets, task, cost=["cls", "box", "mask"]):
        ...
        """
        return self.memory_efficient_forward(outputs, targets, task, cost)
